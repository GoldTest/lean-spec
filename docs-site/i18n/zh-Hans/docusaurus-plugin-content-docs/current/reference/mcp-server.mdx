...
HarnSpec MCP（模型上下文协议）服务器使 Claude Desktop、GitHub Copilot 和其他兼容 MCP 的客户端等 AI 助手能够直接与您的 HarnSpec 项目交互。
...
将 `/path/to/your/harnspec/project` 替换为您的 HarnSpec 项目目录的绝对路径。
...
如果您全局安装了 HarnSpec（`npm install -g harnspec`），可以使用：
...
- **命令**：`npx -y harnspec mcp`（推荐）或 `harnspec mcp`（如果全局安装）
- **工作目录**：您的 HarnSpec 项目根目录
- **传输**：stdio

## 使用示例

配置后，您可以通过 AI 助手与 HarnSpec 项目交互：
...
- 验证配置中的 `cwd` 路径指向有效的 HarnSpec 项目（有 `.harnspec/config.json`）
- 如果使用 `npx`，确保首次运行时有互联网访问
- 如果使用全局安装，检查 `harnspec` 是否在您的 PATH 中
- 尝试直接从项目目录运行 `npx -y harnspec mcp` 或 `harnspec mcp`

### 工具未出现

- 配置更改后重新启动 MCP 客户端
- 检查客户端日志是否有连接错误
- 验证 HarnSpec 项目是否正确初始化（`harnspec init`）
...
- **[使用 AI](/zh-Hans/docs/guide/usage/advanced-features/agent-configuration)** - HarnSpec AI 集成指南
