...
本指南帮助你从现有的 SDD 工具和文档系统迁移到 HarnSpec。

## 快速概述

HarnSpec 迁移主要关注两个方面：

1. **元数据/Frontmatter**（主要）- 使用 `harnspec backfill` 从 git 历史中提取
2. **文件夹组织**（因工具而异）- 取决于你的源工具

**好消息**：HarnSpec 对内容格式很灵活——你可以保留现有的写作风格！
...
**之后手动设置**：
- `priority` - 默认为 'medium'，使用 `harnspec update --priority` 调整
- `tags` - 默认从文件夹名称获取，使用 `harnspec update --tags` 调整
...
对于更复杂的迁移或 AI 辅助工作流，使用 `harnspec migrate` 命令。
...
3. 对于每个文档：
   - 运行：harnspec create <name>
   - 运行：harnspec update <name> --status <status>
   - 运行：harnspec update <name> --priority <priority>
   - 编辑内容以匹配 HarnSpec 结构
4. 保留决策理由和关系
5. 保持 Spec 在 400 行以下
...
- [ ] 安装 HarnSpec：`npm install -g harnspec`
- [ ] 初始化 HarnSpec：`harnspec init`

### 迁移期间

- [ ] 重组文件夹（如果你的源工具需要）
- [ ] 将主 Spec 文件重命名为 `README.md`
- [ ] 运行 `harnspec backfill --assignee --all`
- [ ] 必要时手动调整优先级
- [ ] 必要时手动调整标签
- [ ] 验证状态值是否正确
...
- [ ] 运行 `harnspec validate` 检查问题
- [ ] 运行 `harnspec board` 可视化迁移
- [ ] 测试 Spec 发现：`harnspec list`、`harnspec search`
...
HarnSpec 支持通过 SKILL.md 提供 **Agent Skills**。这样 AGENTS.md 可以保持精简，仅包含项目特定规则。

**推荐步骤：**
1. 通过 `harnspec skill install` 安装 Skill
2. 使用安装后的 SKILL.md：.github/skills/HarnSpec-sdd/
...
1. **审查源工具的现有 AI 指导**
2. **保留项目特定的约定**
3. **与 HarnSpec AGENTS.md 合并**（由 `harnspec init` 创建）
4. **更新命令**：`openspec` → `harnspec` 等
5. **保持团队工作流程完整**
...
**好消息**：HarnSpec 很灵活！保留你现有的格式。

你不需要重写内容来匹配特定结构。HarnSpec 的首要原则是"意图重于实现"——只要 Spec 清晰地传达意图，确切的格式并不重要。
...
- **GitHub Issues**：[报告迁移问题](https://github.com/codervisor/harnspec/issues)
- **示例**：查看 [spec 063](https://harnspec.gitub.io/specs/063-migration-from-existing-tools) 获取详细示例
- **命令帮助**：运行 `harnspec migrate --help` 或 `harnspec backfill --help`
