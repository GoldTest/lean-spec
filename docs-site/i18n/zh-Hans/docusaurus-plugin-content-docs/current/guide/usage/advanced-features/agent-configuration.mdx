...
HarnSpec 现在支持 **Agent Skills** 的 SKILL.md 文件。推荐将 **SDD 方法论** 放入 SKILL.md，让 AGENTS.md 只保留**项目特定规则**。

**项目级 Skill 位置：**

- .github/skills/HarnSpec-sdd/SKILL.md（通过 `harnspec skill install` 安装）
...
如果 MCP 工具不可用，使用 CLI 命令：
    ...
    harnspec board              # 项目概览
    harnspec list               # 查看所有 Spec
    harnspec search "query"     # 查找相关 Spec
    harnspec create <name>      # 创建新 Spec
    harnspec update <spec> --status <status>  # 更新状态
    harnspec deps <spec>        # 显示依赖

**提示：** 使用 CLI 前先检查是否有 HarnSpec MCP 工具可用。
...
**使用 MCP（推荐）：** 配置 HarnSpec MCP 服务器以获得完整功能。参见 [MCP 集成](/zh-Hans/docs/guide/usage/mcp-integration)。
...
这些工具配合 MCP 集成效果最佳。配置 HarnSpec MCP 服务器：
...
显示项目看板。
```
预期：代理使用 MCP `board` 工具（或 `harnspec board` CLI）
