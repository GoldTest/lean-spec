...
title: "HarnSpec 0.2.10：内联元数据编辑、MCP 自动配置和多项目改进"
authors: [marvin]
tags: [release, web-ui, mcp, multi-project]
---

HarnSpec 0.2.10 带来了更具交互性的 Web 体验：内联元数据编辑、初始化时零配置 MCP 设置，以及多项目管理的重大改进。本次发布专注于减少摩擦——无论您是在浏览器中管理 Spec、设置 AI Agent 集成，还是跨多个项目工作。
...
设置 MCP 服务器集成过去需要手动配置。现在 `harnspec init` 会自动处理。

### 自动检测

运行 `harnspec init` 时，HarnSpec 会检测您的 AI 工具并提供 MCP 配置：
...
您的 AI Agent 可以立即访问 HarnSpec 工具——无需从文档复制粘贴，无需排查路径问题。
...
之前，`harnspec ui --port 3002` 会导致依赖页面因硬编码的 localhost:3000 调用而无法工作。已修复——该页面现在支持任意端口。
...
**有想法？** [提交 Issue](https://github.com/codervisor/harnspec/issues) 或[发起讨论](https://github.com/codervisor/harnspec/discussions)。
...
- [完整更新日志](https://github.com/codervisor/harnspec/blob/main/CHANGELOG.md)
