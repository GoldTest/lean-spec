...
Learn how to integrate HarnSpec with AI assistants using the Model Context Protocol (MCP) for seamless spec access and semantic memory retrieval.

## What is MCP?

**Model Context Protocol (MCP)** is an open standard that enables AI assistants to access external tools and data sources. HarnSpec's MCP server lets AI agents:
...
### Option 1: Using @HarnSpec/mcp (Recommended)

The simplest way to use HarnSpec with MCP is via the `@HarnSpec/mcp` package, which provides a dedicated entry point.
...
- ✅ Same functionality as `harnspec mcp`

### Supported Tools

HarnSpec MCP server works with any tool that supports the Model Context Protocol:
...
### Option 2: Direct harnspec Command

If you prefer to use the `harnspec` CLI directly:
...
Expected: AI uses MCP to call `harnspec list` and shows results.
...
3. Project has specs (run `harnspec list` manually)
...
- ✅ Only exposes HarnSpec commands (list, search, view, etc.)
