...
- .github/skills/HarnSpec-sdd/SKILL.md (installed via `harnspec skill install`)
...
If you have HarnSpec MCP tools available, **ALWAYS use them**:
...
If MCP tools are not available, use CLI commands:
    ...
    harnspec deps <spec>        # Show dependencies

**Tip:** Check if you have HarnSpec MCP tools available before using CLI.
...
**With MCP (Recommended):** Configure the HarnSpec MCP server for full functionality. See [MCP Integration](/docs/guide/usage/mcp-integration).
...
These tools work best with MCP integration. Configure the HarnSpec MCP server:
...
```
Expected: Agent uses MCP `board` tool (or `harnspec board` CLI)
