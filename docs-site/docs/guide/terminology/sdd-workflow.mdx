...
SDD is the operating rhythm for HarnSpec: capture intent in a spec, let humans and AI execute from it, and keep the spec in sync.
...
4. **Validate** — run tests and `harnspec validate` to ensure quality.
...
## Tooling Support
- `harnspec list`, `search`, and `view` for discovery.
- `harnspec create` and `update` for planning/status.
- `harnspec validate` and the MCP server for automation.

## See Also
- [Understanding HarnSpec](/docs/guide/understanding-harnspec)
