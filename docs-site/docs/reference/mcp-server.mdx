...
The HarnSpec MCP (Model Context Protocol) server enables AI assistants like Claude Desktop, GitHub Copilot, and other MCP-compatible clients to interact with your HarnSpec projects directly.
...
The MCP server is included with HarnSpec. No additional installation is required.
...
2. Add the HarnSpec MCP server configuration:
...
```json
{
  "mcpServers": {
    "harnspec": {
      "command": "npx",
      "args": ["-y", "harnspec", "mcp"],
      "cwd": "/path/to/your/harnspec/project"
    }
  }
}
```

Replace `/path/to/your/harnspec/project` with the absolute path to your HarnSpec project directory.

### Multiple Projects

You can configure multiple HarnSpec projects:
...
If you have HarnSpec installed globally (`npm install -g harnspec`), you can use:
...
- **Command**: `npx -y harnspec mcp` (recommended) or `harnspec mcp` (if globally installed)
- **Working Directory**: Your HarnSpec project root
- **Transport**: stdio

## Usage Examples

Once configured, you can interact with your HarnSpec project through your AI assistant:
...
- Verify the `cwd` path in your configuration points to a valid HarnSpec project (has `.harnspec/config.json`)
- If using `npx`, ensure you have internet access for the first run
- If using global installation, check that `harnspec` is in your PATH
- Try running `npx -y harnspec mcp` or `harnspec mcp` directly from your project directory

### Tools not appearing

- Restart your MCP client after configuration changes
- Check client logs for connection errors
- Verify the HarnSpec project is properly initialized (`harnspec init`)
...
```bash
npx @modelcontextprotocol/inspector npx -y harnspec mcp
```

Or test directly from your project:

```bash
cd /path/to/your/harnspec/project
npx -y harnspec mcp
...
- It can read and modify files in your HarnSpec project
...
- **[Working with AI](/docs/guide/usage/advanced-features/agent-configuration)** - HarnSpec AI integration guide
