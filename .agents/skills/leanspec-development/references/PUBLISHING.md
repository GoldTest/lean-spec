...
- **Publish platform packages first** (e.g., `harnspec-darwin-arm64`, `@leanspec/mcp-darwin-arm64`)
...
- Packages `harnspec` (CLI), `@leanspec/mcp` (MCP), `@leanspec/ui-components`, `@leanspec/http-server`, and `@leanspec/ui` are published automatically via GitHub Actions when a release is created
