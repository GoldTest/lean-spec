...
The `harnspec create` command creates spec files with a `created` field in the frontmatter. The intended format is `YYYY-MM-DD` (e.g., `2025-11-03`), but after the file is written, the field appears as a full ISO 8601 date string: `2025-11-03T00:00:00.000Z`.
...
- [ ] Create a new spec: `harnspec create test-spec` - verify `created` field is `YYYY-MM-DD`
- [ ] Create with tags: `harnspec create test-spec --tags=foo` - verify `created` remains `YYYY-MM-DD`
- [ ] Create with custom fields: `harnspec create test-spec --field epic=123` - verify format preserved
- [ ] Update existing spec: `harnspec update test-spec --status=in-progress` - verify dates stay formatted
...
harnspec create test-spec --tags=demo
