...
When users run `npx harnspec init --example dark-theme` and prompt "Help me add dark theme support to this app using LeanSpec", the AI doesn't follow proper SDD workflow (using `harnspec create`, `harnspec update --status`) because the generated AGENTS.md lacks proper emphasis.
...
### How Templates Connect to `.harnspec/`

**During `harnspec init`:**

1. **Template selection:**
   - User runs `harnspec init` (uses standard) or `harnspec init --template detailed`
...
2. **Files copied:**
   ...
   └── files/README.md        → .harnspec/templates/spec-template.md
   ```
...
├── AGENTS.md              → project-root/AGENTS.md
└── files/
    ├── README.md          → .harnspec/templates/spec-template.md (or README.md)
    ├── DESIGN.md          → .harnspec/templates/DESIGN.md
    ├── PLAN.md            → .harnspec/templates/PLAN.md
    └── TEST.md            → .harnspec/templates/TEST.md
```

**What stays the same:**

- `.harnspec/config.json` structure (unchanged)
- `.harnspec/templates/` directory purpose (stores spec templates)
- How `harnspec create` reads templates from `.harnspec/templates/`
...
- Existing projects: Keep using `.harnspec/templates/spec-template.md`
- New standard projects: Copy to `.harnspec/templates/spec-template.md` (maintain convention)
- New detailed projects: Copy as `.harnspec/templates/README.md` + other sub-specs
- CLI should check for both `spec-template.md` and `README.md` when creating specs

**When creating new specs:**

- `harnspec create <name>` reads from `.harnspec/templates/spec-template.md` (same as before)
...
2. **`detailed/`**
   ...
   - Opt-in: `harnspec init --template detailed`
...
**Working with specs:**
- `harnspec create <name>` - Create new spec (status: `planned`)
- `harnspec update <spec> --status in-progress` - BEFORE implementing
- `harnspec update <spec> --status complete` - AFTER implementing
...
- `harnspec stats` - Quick metrics

## SDD Workflow

1. **Discover** - Check existing specs: `harnspec list`
2. **Plan** - Create spec: `harnspec create <name>` (status: `planned`)
3. **Start Work** - Mark in-progress BEFORE implementing: `harnspec update <spec> --status in-progress`
4. **Implement** - Write code/docs, keep spec in sync
5. **Complete** - Mark complete AFTER implementation: `harnspec update <spec> --status complete`
...
- [ ] Update file copying logic to handle new layout:
  ...
  - Copy `{template}/files/*` → `.harnspec/templates/`
...
- [ ] Update `harnspec create` to support both naming conventions:
...
- [ ] Test `harnspec init` (should use standard)
- [ ] Test `harnspec init --template enterprise`
- [ ] Test `harnspec init --template minimal` (should show error or default to standard)
- [ ] Run tutorial test: `npx harnspec init --example dark-theme`
...
- [ ] `harnspec init` uses standard template
- [ ] AGENTS.md copied from `standard/AGENTS.md` to project root
- [ ] Standard: `files/README.md` copied as `.harnspec/templates/spec-template.md` (backward compat)
- [ ] Detailed: All files copied preserving names to `.harnspec/templates/`
- [ ] `harnspec create` works with both `spec-template.md` and `README.md`
- [ ] `harnspec init --template detailed` copies AGENTS.md + sub-spec files
- [ ] Existing projects with `spec-template.md` continue working
- [ ] `harnspec init --template minimal` handles gracefully (error or default to standard)
- [ ] `harnspec init --template enterprise` handles gracefully (suggest detailed instead)
...
- [ ] `harnspec create` checks for `spec-template.md` first, then `README.md`
...
- [ ] Run `npx harnspec init --example dark-theme`
- [ ] Check AGENTS.md has strong CLI command emphasis
- [ ] Prompt AI: "Help me add dark theme support using LeanSpec"
- [ ] Verify AI uses `harnspec create` command (not manual file creation)
- [ ] Verify AI uses `harnspec update --status in-progress` before implementing
- [ ] Verify AI uses `harnspec update --status complete` after implementing
