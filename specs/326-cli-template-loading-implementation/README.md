...
**Problem**: The CLI `create` command currently uses a compile-time embedded template instead of loading templates from `.harnspec/templates/`. This means:

- User template customizations in `.harnspec/templates/spec-template.md` are completely ignored
...
Initialize TemplateLoader with config
  ↓
Load template from .harnspec/templates/
  ↓
...
fn load_config(project_root: &Path) -> Result<Config, Box<dyn Error>> {
    let config_path = project_root.join(".harnspec/config.json");
...
- [x] Phase 4: Verification
  - [x] Verify CLI uses `.harnspec/templates/spec-template.md`
...
- [x] CLI loads default template from `.harnspec/templates/spec-template.md`
...
- [x] Clear error when `.harnspec/` not found
- [x] Clear error when template file missing
- [x] Clear error when template format invalid
- [x] Suggests running `harnspec init` when appropriate
...
- Projects without `.harnspec/templates/` get helpful error (not silent failure)
...
4. **Implemented helper functions**:
   - `find_project_root()` - Walks up directory tree to find `.harnspec/`
...
- ✅ Default template loading from `.harnspec/templates/spec-template.md`
...
- Customize templates in `.harnspec/templates/spec-template.md`
