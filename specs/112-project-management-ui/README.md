...
4. **Directory Picker UX Improvements**:
    * **Default CWD**: Set the initial directory to the user's current working directory (where `harnspec ui` was launched), not the home directory
...
* Most users launch `harnspec ui` from their project directory or workspace root
...
* Project validity should be based on `.harnspec/` presence
