...
```bash
harnspec <spec>              # Default: formatted view (current "show")
harnspec <spec> --raw        # Raw markdown (current "read")  
harnspec <spec> --json       # JSON output (current "read --format=json")
harnspec <spec> --edit       # Open in editor (current "open")
```
...
```bash
harnspec view <spec>         # Formatted (remove "show", keep only "view")
harnspec view <spec> --raw   # Raw markdown
harnspec view <spec> --json  # JSON output
harnspec open <spec>         # Edit (separate action, keep separate)
```
...
program
  ...
  .description('[DEPRECATED] Use: harnspec <spec-path>')
  .action(async (specPath: string) => {
    console.warn(chalk.yellow('⚠️  "harnspec show" is deprecated. Use: harnspec <spec-path>'));
...
- [x] Update MCP server to align with CLI (harnspec_read → harnspec_view with flags)
- [x] Update all tests
...
- [x] `harnspec view <spec>` displays formatted spec
- [x] `harnspec view <spec> --raw` outputs raw markdown (pipeable)
- [x] `harnspec view <spec> --json` outputs valid JSON
- [x] `harnspec open <spec>` opens in editor
...
- Natural evolution: `harnspec list` → `harnspec <spec>` (browse → view)
...
- Users should update to: `harnspec view <spec>` (with optional `--raw` or `--json` flags)
