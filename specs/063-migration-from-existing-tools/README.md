...
Add `harnspec migrate` command to help users migrate from mainstream SDD tools to LeanSpec.
...
- **Solution**: Use `harnspec backfill` to extract metadata from git history + existing docs
...
- Use `harnspec backfill` to extract metadata from git history and existing docs
