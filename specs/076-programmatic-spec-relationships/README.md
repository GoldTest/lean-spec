...
- `harnspec deps <spec>` - Views relationships ✅
...
harnspec create new-feature --priority high
# Now AI must:
# 1. Read the file
...
harnspec deps add 076 --depends-on 073,074

# Add related specs (informational)
harnspec deps add 076 --related 072,025

# Combined
harnspec deps add 076 --depends-on 073 --related 072,074
```
...
harnspec deps remove 076 --depends-on 073

# Remove specific related
harnspec deps remove 076 --related 072

# Remove all dependencies
harnspec deps remove 076 --depends-on --all

# Remove all relationships
harnspec deps remove 076 --all
```

#### View Relationships (already exists)

```bash
harnspec deps 076  # Already implemented ✅
```
...
**1. Generic `harnspec update` extension**:

```bash
harnspec update 076 --depends-on 073,074
```
...
```bash
harnspec deps-add 076 --depends-on 073
harnspec deps-remove 076 --related 072
```
...
- [ ] **MCP Implementation**
  - [ ] Add `mcp_harnspec_deps-add` tool
  - [ ] Add `mcp_harnspec_deps-remove` tool
...
Before (manual edit):
1. Create spec: harnspec create new-feature
2. Read README.md
3. Parse YAML
4. Edit frontmatter
5. Write back
...
After (programmatic):
1. Create spec: harnspec create new-feature  
2. Add deps: harnspec deps add new-feature --depends-on 073,074
...
1. **Bulk operations**: `harnspec deps add 076 --depends-on $(harnspec list --status planned --format ids)`
