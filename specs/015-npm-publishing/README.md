...
- Package published to npm as `harnspec`
- Users can install via `npm install -g harnspec` or `pnpm add -D harnspec`
...
- ✅ Name: `harnspec` (check npm availability)
...
- ⚠️ .npmignore: Exclude dev files (src/, coverage/, .harnspec/, specs/)
...
- [x] Test CLI after build: `node bin/harnspec.js --help`
...
- [x] Review package contents: `tar -tzf harnspec-*.tgz`
...
- [ ] Verify on npm: <https://www.npmjs.com/package/harnspec>
- [ ] Test installation: `npm install -g harnspec`
- [ ] Test CLI works: `harnspec --version`
...
- [x] No sensitive files included (.env, .harnspec/, specs/) ✅
- [x] bin/harnspec.js has correct shebang: `#!/usr/bin/env node` ✅

### Post-Publishing Tests

- [x] `npm link` for local testing ✅
- [x] `harnspec --version` shows correct version ✅ 0.1.0
- [x] `harnspec --help` displays help ✅
- [x] `harnspec init` works in fresh directory ✅
- [x] `harnspec create test` creates a spec ✅
- [x] All commands work after global install ✅
- [ ] `npm install -g harnspec` works (after publishing)
...
- ✅ Package name `harnspec` confirmed available on npm
...
- Unscoped: `harnspec` (requires name availability)
- Scoped: `@codervisor/harnspec` (guaranteed availability under your org)
