...
```bash
harnspec restore <spec>           # Restore from archived/
harnspec restore archived/045-*   # Support both paths

# With preview
harnspec restore <spec> --dry-run

# Alternative naming (pick one)
harnspec unarchive <spec>         # More explicit
harnspec restore <spec>           # More intuitive
```
...
|----------|-------|-------------|
| Spec not in archived/ | `Error: Spec not found in archived/` | Check path with `harnspec list --status archived` |
...
- [ ] `harnspec restore archived/045-feature` - Basic restore
- [ ] `harnspec restore 045-feature` - Auto-detect archived prefix
- [ ] `harnspec restore archived/045-*` - Glob pattern support
- [ ] `harnspec restore active-spec` - Error: spec not archived
- [ ] `harnspec restore archived/nonexistent` - Error: not found
...
```bash
# Archive
harnspec archive 045-feature
# Status: complete → archived
# Location: specs/ → specs/archived/

# Restore
harnspec restore archived/045-feature
# Status: archived → planned
...
```bash
# List all archived
harnspec list --status archived

# View archived spec
harnspec view archived/045-feature

# Search includes archived (maybe add flag?)
harnspec search "feature" --include-archived
```

### Future Enhancements

1. **Bulk operations**: `harnspec restore --all --before 2025-01-01`
