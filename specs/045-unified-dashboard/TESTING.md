...
- [ ] `harnspec stats` outputs same format as before (no breaking changes)
- [ ] `harnspec stats --json` matches existing JSON schema
- [ ] All existing filter options work (--tag, --assignee, --priority)

### New Timeline Integration

- [ ] `harnspec stats --timeline` adds timeline section after stats
- [ ] `harnspec stats --history` shows full historical view
- [ ] `harnspec stats --velocity` shows cycle time analysis
- [ ] `harnspec stats --all` combines everything
- [ ] Timeline data matches previous `harnspec timeline` output
...
- [ ] Suggests `harnspec init` to get started
...
- [ ] `harnspec` defaults to dashboard
- [ ] `harnspec dashboard` explicitly shows dashboard
- [ ] Dashboard → `harnspec list` → dashboard (workflow)
- [ ] Dashboard respects .harnspec/config.json

#### Cross-Command Consistency

- [ ] Dashboard counts match `harnspec list` counts
- [ ] Dashboard stats match `harnspec stats` output
- [ ] Dashboard activity matches `harnspec stats --timeline`
...
- [ ] `harnspec stats` backward compatible
...
- [ ] New user runs `harnspec`, immediately understands project state
