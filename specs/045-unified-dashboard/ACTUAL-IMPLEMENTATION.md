...
1. **Dashboard Command** - Decided against creating a separate command
   ...
   - No `harnspec` default to dashboard (keeps help accessible)
...
2. **Discoverability**: `harnspec --help` remains the entry point (not a dashboard)
...
- Enhanced `harnspec stats` with multiple view modes
- Enhanced `harnspec board` with velocity insights
