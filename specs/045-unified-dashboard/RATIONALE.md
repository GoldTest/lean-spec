...
Run `harnspec` (no arguments) to see a comprehensive project overview!
...
`harnspec stats` now supports timeline views:
- `harnspec stats --timeline` - add 14-day activity
- `harnspec stats --history` - full historical view
- `harnspec stats` - current stats only (unchanged)

The standalone `harnspec timeline` command is deprecated and will be
removed in v0.4.0. Use `harnspec stats --history` instead.
```
...
```bash
harnspec analytics           # New command
harnspec analytics --stats   # Current stats
harnspec analytics --timeline # Timeline view
```
...
```bash
harnspec stats               # Current behavior (default)
harnspec stats --timeline    # Add timeline
harnspec stats --history     # Full timeline focus
```
