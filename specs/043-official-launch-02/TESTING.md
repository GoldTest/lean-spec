...
- [ ] Install via `npm install -g harnspec` works
- [ ] `harnspec init` completes successfully
- [ ] `harnspec create` generates valid specs
- [ ] `harnspec list` displays correctly
- [ ] `harnspec search` returns relevant results
- [ ] `harnspec update` modifies specs correctly
- [ ] `harnspec board` renders properly
- [ ] `harnspec stats` shows accurate data
...
- [ ] No warnings from `harnspec complexity` (when implemented)
...
- [ ] `harnspec list` < 100ms (for repos with <100 specs)
- [ ] `harnspec search` < 500ms (full-text search)
- [ ] `harnspec view` < 50ms (single spec)
- [ ] `harnspec board` < 200ms (with stats)
- [ ] `harnspec stats` < 300ms (full analytics)
